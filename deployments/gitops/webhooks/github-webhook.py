...
def trigger_argocd_sync(application_name, repository_url, branch='main'):
    ...
    try:
        ...
        logger.error(f"Timeout syncing application: {application_name}")
        return {
            'status': 'error',
            'message': f'Timeout syncing application {application_name}'
        }
    except Exception as e:
        logger.error(f"Exception syncing application {application_name}: {str(e)}")
        return {
            'status': 'error',
...
def get_affected_applications(repository_url, changed_files):
    ...
    app_mappings = {
        'deployments/gitops/applications/infrastructure.yaml': ['metallb', 'cert-manager', 'ingress-nginx', 'longhorn'],
...
def health_notification():
    ...
    logger.warning(f"Health issue in application: {payload.get('application')} - {payload.get('health')}")
