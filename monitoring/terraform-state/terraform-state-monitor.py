...
class TerraformStateMonitor:
    ...
    def get_state_file(self, environment: str, workspace: str = 'default') -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"Failed to get state for {environment}/{workspace}: {e}")
            return None
    
    def _get_s3_state(self, environment: str, workspace: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"Failed to get S3 state {bucket}/{key}: {e}")
            return None
    
    def _get_local_state(self, environment: str, workspace: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"Failed to read local state {state_path}: {e}")
            return None
    
    def analyze_state(self, state: Dict, environment: str, workspace: str):
        ...
        logger.info(f"State analysis for {environment}/{workspace}: "
                   f"{len(resources)} resources, {len(resource_types)} types")
    
    def check_drift(self, environment: str, workspace: str):
