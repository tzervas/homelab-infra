...
class CertificateMonitor:
    ...
    def __init__(self, config_file: str = "/etc/security/cert-monitoring-config.yaml") -> None:
    ...
    def get_kubernetes_certificates(self) -> list[Certificate]:
        ...
        try:
            ...
            logger.exception(f"Failed to parse certificate JSON: {e}")

        return certificates

    def _parse_k8s_certificate(self, cert_data: dict) -> Certificate | None:
                ...
                if condition.get("type") == "Ready" and condition.get("status") == "True":
                    ...
                    if secret_exit_code == 0:
                        try:
                            ...
                            logger.exception(f"Failed to parse secret data for {name}: {e}")

        except Exception as e:
            logger.exception(f"Error parsing certificate data: {e}")

        return None

    def _parse_certificate_data(self, cert_data_b64: str) -> dict | None:
        ...
        try:
            ...
            logger.exception(f"Failed to parse certificate data: {e}")
            return None
    ...
    def check_file_certificates(self, cert_paths: list[str]) -> list[Certificate]:
        ...
        for cert_path in cert_paths:
            try:
                ...
                logger.exception(f"Failed to check certificate {cert_path}: {e}")

        return certificates

    def _get_file_certificate_info(self, cert_path: str) -> Certificate | None:
        ...
        try:
            ...
            logger.exception(f"Error getting file certificate info: {e}")
            return None
    ...
    def check_remote_certificates(self, hosts: list[dict[str, str]]) -> list[Certificate]:
        ...
        for host_config in hosts:
            ...
            try:
                ...
                logger.exception(f"Failed to check remote certificate {hostname}:{port}: {e}")

        return certificates

    def _get_remote_certificate_info(self, hostname: str, port: int) -> Certificate | None:
        ...
        try:
            ...
            logger.exception(f"Error getting remote certificate info: {e}")
            return None
    ...
    def _create_email_body(self, certificates: list[Certificate]) -> str:
        ...
        critical_certs = [
            c for c in certificates if c.days_until_expiry <= self.alert_config.critical_days
        ]
        warning_certs = [
            c
            for c in certificates
            if self.alert_config.critical_days
            < c.days_until_expiry
            <= self.alert_config.warning_days
        ]
                ...
                "certificates": [
                    {
                        ...
                        "severity": "critical"
                        if cert.days_until_expiry <= self.alert_config.critical_days
                        else "warning",
                    }
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        default="/etc/security/cert-monitoring-config.yaml",
    ...
    try:
        ...
        logger.exception(f"Certificate monitoring failed: {e}")
        sys.exit(1)
