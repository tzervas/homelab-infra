...
class SecurityComplianceChecker:
    ...
    def __init__(self, config_file: str = "/etc/security/security-compliance-config.yaml"):
    ...
    def _execute_check(self, policy: Policy) -> (bool, str):
        ...
        try:
            result = subprocess.run(policy.check_command, capture_output=True, text=True, timeout=30)
            passed = policy.expected_output in result.stdout
            return passed, result.stdout
        except subprocess.TimeoutExpired:
            logger.error(f"Policy check '{policy.name}' timed out")
            return False, 'Timeout'
        except Exception as e:
            logger.error(f"Error executing policy check '{policy.name}': {e}")
            return False, str(e)
    ...
    def run_compliance_checks(self, report_file: str = '/var/log/security-compliance-report.txt') -> None:
...
def main():
    ...
    parser.add_argument('--config', default='/etc/security/security-compliance-config.yaml',
                        help='Configuration file path')
    parser.add_argument('--report', default='/var/log/security-compliance-report.txt',
