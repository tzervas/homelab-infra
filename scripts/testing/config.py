...
class TestingConfig:
    ...
    critical_namespaces: List[str] = field(default_factory=lambda: [
        "kube-system", "metallb-system", "cert-manager",
        "ingress-nginx", "longhorn-system", "monitoring"
    ])

    # Network policies to validate
    network_policies: List[str] = field(default_factory=lambda: [
        "default-deny-all", "allow-dns", "monitoring-ingress"
    ])

    # Service endpoints for integration testing
    service_endpoints: Dict[str, ServiceEndpoint] = field(default_factory=lambda: {
    ...
    service_definitions: Dict[str, ServiceDefinition] = field(default_factory=lambda: {
    ...
    validation_schemas: Dict[str, Dict[str, Any]] = field(default_factory=lambda: {
        "ansible_inventory": {
            "type": "object",
            "required": ["all"],
            "properties": {
    ...
    default_config_paths: List[str] = field(default_factory=lambda: [
        "ansible/inventory",
        "helm/environments",
    ...
    service_communication_tests: List[tuple] = field(default_factory=lambda: [
...
def update_service_endpoints_from_env() -> Dict[str, ServiceEndpoint]:
    ...
    env_mappings = {
        "gitlab": {
            "internal_url": "GITLAB_INTERNAL_URL",
