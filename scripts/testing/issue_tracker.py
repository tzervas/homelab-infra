...
class IssueTracker:
    ...
    def __init__(self, max_issues_per_component: int = 5, max_total_display: int = 20):
    ...
    def add_issue(self, 
                  ...
                  affects_deployment: bool = False) -> None:
    ...
    def add_issues_from_list(self,
                           component: str,
                           issues_list: List[str],
                           severity: IssueSeverity,
                           category: IssueCategory,
                           max_display: Optional[int] = None) -> None:
    ...
    def categorize_security_issue(self, issue_description: str) -> Tuple[IssueSeverity, IssueCategory]:
        ...
        if any(term in description_lower for term in [
            "cluster-admin", "privileged", "root user", "privilege escalation",
            "cluster role binding", "security context missing"
        ]):
            return IssueSeverity.CRITICAL, IssueCategory.SECURITY
        
        # High security issues
        if any(term in description_lower for term in [
            "runasnonroot", "capabilities", "seccomp", "pod security",
            "rbac", "service account token"
        ]):
            return IssueSeverity.HIGH, IssueCategory.SECURITY
        
        # Medium security issues
        if any(term in description_lower for term in [
            "network policy", "tls", "certificate", "encryption"
        ]):
            return IssueSeverity.MEDIUM, IssueCategory.SECURITY
        
        return IssueSeverity.LOW, IssueCategory.SECURITY
    
    def categorize_deployment_issue(self, issue_description: str) -> Tuple[IssueSeverity, IssueCategory]:
        """Automatically categorize deployment-related issues."""
        description_lower = issue_description.lower()
        
        # Critical deployment issues
        if any(term in description_lower for term in [
            "failed", "error", "crash", "cannot connect", "unreachable",
            "deployment user does not exist", "kubeconfig not found"
        ]):
            return IssueSeverity.CRITICAL, IssueCategory.DEPLOYMENT
        
        # High deployment issues
        if any(term in description_lower for term in [
            "not ready", "pending", "timeout", "missing", "configuration"
        ]):
            return IssueSeverity.HIGH, IssueCategory.DEPLOYMENT
        
        return IssueSeverity.MEDIUM, IssueCategory.DEPLOYMENT
    
    def auto_categorize_issue(self, component: str, message: str) -> Tuple[IssueSeverity, IssueCategory]:
        """Automatically categorize an issue based on component and message."""
        message_lower = message.lower()
        component_lower = component.lower()
        
        # Security-related components and messages
        if any(term in component_lower for term in ["security", "rbac", "tls", "cert"]) or \
           any(term in message_lower for term in ["security", "privilege", "root", "capabilities"]):
            return self.categorize_security_issue(message)
        
        # Deployment-related issues
        if any(term in component_lower for term in ["deployment", "user", "ansible"]) or \
           any(term in message_lower for term in ["deploy", "install", "configure"]):
            return self.categorize_deployment_issue(message)
        
        # Network/connectivity issues
        if any(term in component_lower for term in ["network", "dns", "metallb", "ingress"]) or \
           any(term in message_lower for term in ["connect", "network", "dns", "timeout"]):
        ...
        # Performance issues
        if any(term in message_lower for term in ["slow", "performance", "resource", "memory", "cpu"]):
    ...
    def add_auto_categorized_issue(self,
                                 component: str,
                                 message: str,
                                 details: Optional[Dict[str, Any]] = None,
        ...
        affects_deployment = (
            severity in [IssueSeverity.CRITICAL, IssueSeverity.HIGH] and
            category in [IssueCategory.SECURITY, IssueCategory.DEPLOYMENT, IssueCategory.CONNECTIVITY]
        )
    ...
    def format_summary_report(self, summary: Optional[IssueSummary] = None) -> str:
        ...
        if blocking_issues and len(blocking_issues) > len(critical_issues):
            ...
            non_critical_blocking = [i for i in blocking_issues if i.severity != IssueSeverity.CRITICAL]
            for issue in non_critical_blocking[:10]:  # Limit to top 10
                report.append(f"- **{issue.component}**: {issue.message}")
            
            if len(non_critical_blocking) > 10:
                report.append(f"- ... and {len(non_critical_blocking) - 10} more deployment blocking issues")
            report.append("")
        ...
        if summary.deployment_blocking > 0:
            ...
            report.append("1. **Immediate Action Required**: Fix critical and deployment-blocking issues first")
...
def create_missing_items_issues(tracker: IssueTracker,
                               component: str,
                               missing_items: List[str],
                               item_type: str,
                               severity: IssueSeverity = IssueSeverity.HIGH,
                               category: IssueCategory = IssueCategory.CONFIGURATION) -> None:
...
def create_validation_failure_issues(tracker: IssueTracker,
                                    component: str,
                                    failed_validations: List[str],
                                    validation_type: str = "validation") -> None:
...
def create_security_context_issues(tracker: IssueTracker,
                                 component: str,
                                 privileged_containers: List[str],
                                 missing_contexts: List[str]) -> None:
