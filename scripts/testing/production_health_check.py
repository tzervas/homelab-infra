...
class ProductionHealthChecker:
    ...
    def check_cluster_connectivity(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'message': 'Kubernetes cluster is accessible' if success else f'Cluster not accessible: {output}',
            'details': output
        }
    ...
    def check_critical_namespaces(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'message': f'All critical namespaces are active' if not issues else f'Issues with namespaces: {", ".join(issues)}',
                'details': {
    ...
    def check_deployment_health(self) -> Dict[str, Any]:
        ...
        success, output = self.run_kubectl_command(['get', 'deployments', '--all-namespaces', '-o', 'json'])
        if not success:
            return {
    ...
    def check_service_endpoints(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'message': 'All critical services have endpoints' if not endpoint_issues else f'Endpoint issues: {", ".join(endpoint_issues)}',
            'details': {
    ...
    def check_ingress_connectivity(self) -> Dict[str, Any]:
        ...
        success, output = self.run_kubectl_command([
            'get', 'svc', 'ingress-nginx-controller', '-n', 'ingress-nginx', '-o', 'json'
        ])
        ...
        try:
            ...
            return {
                ...
                'message': 'Ingress has external IPs' if has_external_access else 'Ingress service exists but no external IPs',
                'details': {
                    'service_type': service_data['spec']['type'],
                    'external_ips': [ip.get('ip', ip.get('hostname', 'unknown')) for ip in ingress_ips]
                }
            }
        ...
        try:
            ...
            for cert in certificates:
                ...
                if is_ready:
                    ...
                    reason = ready_condition.get('reason', 'Unknown') if ready_condition else 'No Ready condition'
    ...
    def check_monitoring_stack(self) -> Dict[str, Any]:
        ...
        for component in monitoring_components:
            success, output = self.run_kubectl_command([
                'get', 'deployment', component, '-n', 'monitoring', '-o', 'json'
            ])
            ...
            try:
                ...
                component_status[component] = {
                    ...
                    'message': 'Running normally' if is_healthy else f'Only {ready}/{desired} replicas ready'
                }
    ...
    def run_all_checks(self) -> Dict[str, Any]:
        ...
        if failed == 0 and warnings == 0:
            ...
            self.results['overall_message'] = f'Production health check failed: {failed} failed checks, {warnings} warnings'
    ...
    def _output_junit(self) -> None:
        ...
        print('<testsuites>')
        print(f'<testsuite name="ProductionHealthCheck" tests="{self.results["summary"]["total_checks"]}" '
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Production Health Check for Homelab Infrastructure')
    parser.add_argument(
        '--output-format',
