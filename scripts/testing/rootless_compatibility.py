...
try:
    from .common import setup_logger, safe_import
...
class RootlessCompatibilityChecker:
    ...
    def __init__(self, kubeconfig_path: Optional[str] = None, log_level: str = "INFO"):
        ...
        self.deployment_mode = os.getenv("HOMELAB_DEPLOYMENT_MODE", "vm-based")  # vm-based or bare-metal
        ...
        try:
            ...
            if Path("/proc/cpuinfo").exists():
                with open("/proc/cpuinfo", "r") as f:
                    cpuinfo = f.read()
                    if any(virt in cpuinfo.lower() for virt in ["kvm", "qemu", "hypervisor", "virtualization"]):
            ...
            if subprocess.run(["which", "virsh"], capture_output=True).returncode == 0:
        ...
        try:
            if kubeconfig_path:
                ...
                try:
                    ...
                    self.logger.warning("No direct K8s access from bastion - normal for VM-based deployment")
                    return
            else:
                config.load_kube_config()
    ...
    def check_deployment_user_configuration(self) -> RootlessCompatibilityResult:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                recommendations.append("Run setup-secure-deployment.sh to create the deployment user")
            else:
    ...
    def check_kubernetes_security_contexts(self) -> RootlessCompatibilityResult:
        ...
        try:
            ...
            if total_privileged > 0:
                issues.append(f"Found {total_privileged} deployments/containers with privileged access")
            ...
            if total_missing > 0:
                issues.append(f"Found {total_missing} deployments/containers without security contexts")
                if total_missing > 5:
            ...
            if total_pss_missing > 0:
                issues.append(f"Found {total_pss_missing} namespaces without Pod Security Standards")
                if total_pss_missing > 5:
                    issues.append(f"Missing PSS namespaces (showing 5): {', '.join(namespaces_without_pss[:5])}")
                recommendations.extend([
        ...
        try:
            ...
            for values_file in values_files:
                try:
                    ...
                    if any(skip in str(values_file) for skip in ['template', 'example', '.template']):
                        continue
                        
                    with open(values_file, 'r') as f:
                        content = f.read()
                    
                    has_security_config = any(pattern in content for pattern in security_context_patterns)
            ...
            if total_files_without_security > 0:
                issues.append(f"Found {total_files_without_security} Helm values files without security contexts")
                if total_files_without_security > 5:
                    issues.append(f"Files missing security contexts (showing 5): {', '.join(files_without_security[:5])}")
                recommendations.extend([
    ...
    def check_vm_deployment_readiness(self) -> RootlessCompatibilityResult:
        ...
        try:
            ...
            virsh_result = subprocess.run(["which", "virsh"], capture_output=True)
            if virsh_result.returncode != 0:
                ...
                if vm_list_result.returncode == 0:
                    vm_count = len([line for line in vm_list_result.stdout.split('\n') if 'test-vm' in line])
            ...
            ip_result = subprocess.run(["ip", "addr", "show", "virbr0"], capture_output=True, text=True)
    ...
    def run_comprehensive_compatibility_check(self) -> List[RootlessCompatibilityResult]:
        ...
        self.logger.info(f"Starting comprehensive rootless compatibility check for {self.deployment_mode} deployment...")
        ...
        if self.deployment_mode in ["bastion-host", "vm-based"]:
            ...
            self.logger.warning(f"Unknown deployment mode '{self.deployment_mode}' - running all checks")
        ...
        for check in checks:
            try:
                ...
                self.logger.info(f"{status_icon} {result.component}: {'Compatible' if result.compatible else 'Issues found'}")
                ...
                results.append(RootlessCompatibilityResult(
                    component="unknown",
                    compatible=False,
