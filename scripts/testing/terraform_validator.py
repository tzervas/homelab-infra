...
class TerraformResource:
    ...
    def is_valid(self) -> bool:
        ...
        return len(self.instances) > 0 and all(
            instance.get("attributes", {}) is not None
...
class TerraformStateValidator:
    ...
    def _run_terraform_command(self, command: List[str]) -> Optional[str]:
        ...
        try:
            ...
            self.logger.error(f"Terraform command timed out: {' '.join(command)}")
            return None
    ...
    def validate_kubernetes_resources(self) -> TerraformValidationResult:
        ...
        for expected_ns in expected_namespaces:
            found = any(
                expected_ns in r.instances[0].get("attributes", {}).get("metadata", [{}])[0].get("name", "")
        ...
        return TerraformValidationResult(
            ...
            details={
                ...
                "resource_types": list(set(r.type for r in k8s_resources)),
            },
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Validate Terraform state for homelab infrastructure")
    parser.add_argument("--terraform-dir", default="terraform", help="Path to Terraform directory")
    parser.add_argument("--log-level", default="INFO", choices=["DEBUG", "INFO", "WARNING", "ERROR"])
    parser.add_argument("--component", choices=["state", "kubernetes", "helm"], help="Validate specific component only")
    ...
    for result in results:
        ...
        if result.resources_checked > 0:
            ...
            print(f"    📊 Resources: {result.resources_valid}/{result.resources_checked} ({percentage:.1f}%)")
