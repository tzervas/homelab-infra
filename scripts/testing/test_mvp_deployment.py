...
from homelab_orchestrator.core.config_manager import ConfigManager, ConfigContext
...
class MVPDeploymentTest:
    ...
    def test_security_configuration(self) -> bool:
        ...
        try:
            ...
            if oauth2_file.exists():
                ...
                if "aG9tZWxhYi1wb3J0YWwtc2VjcmV0" in content or "SEQzeUMxN1JmR3VYVGg4dXRHWXNLRGZLYUJ2bDhqUXM=" in content:
                    self.logger.error("❌ Found hardcoded secrets in OAuth2 configuration")
                    return False
                
                if "${OAUTH2_CLIENT_SECRET}" not in content or "${OAUTH2_COOKIE_SECRET}" not in content:
    ...
    def test_version_information(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"❌ Version test failed: {e}")
            return False
