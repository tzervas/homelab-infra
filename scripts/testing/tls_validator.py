...
class TLSSecurityValidator:
    ...
    def _parse_certificate(self, cert_der: bytes) -> Optional[CertificateInfo]:
        ...
        if not CRYPTO_AVAILABLE:
            self.logger.warning("Cryptography library not available for detailed certificate parsing")
            return None
    ...
    def _get_ssl_info(self, hostname: str, port: int, timeout: int = 10) -> Tuple[Optional[ssl.SSLSocket], Optional[CertificateInfo]]:
        ...
        try:
            ...
            self.logger.error(f"Failed to get SSL info for {hostname}:{port} - {e}")
            return None, None
    ...
    def validate_endpoint_tls(self, endpoint: str) -> TLSValidationResult:
        ...
        if cipher:
            ...
            if missing_components:
                warnings.append(f"Cipher missing recommended components: {', '.join(missing_components)}")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if api_url:
                    ...
                    results.append(TLSValidationResult(
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Validate TLS/mTLS security for homelab infrastructure")
    parser.add_argument("--log-level", default="INFO", choices=["DEBUG", "INFO", "WARNING", "ERROR"])
    parser.add_argument("--endpoint", help="Specific endpoint to validate")
    parser.add_argument("--component", choices=["kubernetes", "ingress"], help="Validate specific component only")
    
    args = parser.parse_args()
    
    if not CRYPTO_AVAILABLE:
        print("Warning: cryptography library not available. Install with: pip install cryptography requests")
