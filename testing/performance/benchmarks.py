...
class PerformanceReport:
    ...
    def finalize(self) -> None:
        ...
        self.summary_stats = {
            ...
            "avg_response_time_ms": statistics.mean([r.value for r in successful_tests if r.unit == "ms"]) if successful_tests else 0
        }
...
class InfrastructureBenchmarker:
    ...
    def benchmark_storage_performance(self) -> List[BenchmarkResult]:
        ...
        try:
            ...
            results.append(BenchmarkResult(
                ...
                unit="count",
                metadata={"storage_classes": [sc.metadata.name for sc in storage_classes.items]}
            ))
                ...
                while not pvc_bound and (time.time() - check_start) < timeout:
                    try:
                        pvc_status = v1.read_namespaced_persistent_volume_claim(
                            name="benchmark-test-pvc", namespace="default"
                        )
    ...
    def run_load_test(self, target_url: str, duration_seconds: int = 60, 
                     concurrent_users: int = 10) -> List[BenchmarkResult]:
        """Run a load test against a target URL."""
        results = []
        
        self.logger.info(f"Starting load test: {target_url} for {duration_seconds}s with {concurrent_users} users")
    ...
    def run_comprehensive_benchmark(self, 
                                  ...
                                  load_test_targets: Optional[List[str]] = None) -> PerformanceReport:
        ...
        report.finalize()
        self.logger.info(f"✅ Benchmark suite completed in {report.total_duration_seconds:.2f} seconds")
    ...
    def _export_markdown_report(self, report: PerformanceReport, output_file: str) -> None:
        ...
        with open(output_file, 'w') as f:
            ...
            f.write(f"- **Average Response Time:** {stats.get('avg_response_time_ms', 0):.2f}ms\n\n")
            ...
            components = list(set(r.component for r in report.results))
            for component in sorted(components):
                ...
                for result in component_results:
                    ...
                    f.write(f"| {result.test_name} | {result.metric_name} | {result.value:.2f} | {result.unit} | {status} |\n")
                f.write("\n")
