...
class CertificateValidator:
    ...
    def get_certificate_from_endpoint(self, hostname: str, port: int = 443) -> CertificateInfo:
        ...
        try:
            ...
            self.logger.exception(f"Failed to get certificate from {hostname}:{port}: {e}")
            raise
        ...
        try:
            ...
            try:
                ...
                if api_server_url.startswith("https://"):
                    ...
                    port = (
                        int(api_server_url.split(":")[2])
                        if ":" in api_server_url.split("//")[1]
                        else 443
                    )
            ...
            self.logger.exception(f"Failed to validate Kubernetes certificates: {e}")
