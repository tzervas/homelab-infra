...
def test_validate_dependencies_with_all_deps_available(test_env):
    ...
    test_config = {"dependencies": ["dep1", "dep2"]}
    with patch.object(test_env, "get_available_resources", return_value={"dep1", "dep2", "dep3"}):
...
def test_managed_fixture_cleanup(test_env):
    ...
    with patch.object(test_env, "create_fixture", side_effect=[fixture1, fixture2]) as mock_create:
        fixture_config = {
            "fixtures": [
                {"type": "test1"},
                {"type": "test2"},
...
def test_create_fixture_not_implemented(test_env):
    ...
    with pytest.raises(
        NotImplementedError, match="Creation of fixture type 'test' not implemented"
    ):
...
def test_managed_fixture_cleanup_failure(test_env):
    ...
    with patch.object(
        test_env, "create_fixture", side_effect=[fixture1, Exception("Creation failed")]
    ) as mock_create:
        fixture_config = {
            "fixtures": [
                {"type": "test1"},
